...
try {
    ...
    if ($_FILES['photoUpload']['name'] != '') {
        ...
        $sql = 'UPDATE travelimage SET PATH = :path,Title = :title, Description = :description,CityCode = :citycode,Country_RegionCodeISO = :iso,Content = :content WHERE ImageID = :ImageID';
        $result = $pdo->prepare($sql);
        $result->bindValue(':ImageID',$_POST['ImageID']);
        ...
        $sql = 'UPDATE travelimage SET Title = :title, Description = :description,CityCode = :citycode,Country_RegionCodeISO = :iso,Content = :content WHERE ImageID = :ImageID';
        $result = $pdo->prepare($sql);
        $result->bindValue(':ImageID',$_POST['ImageID']);
        $result->bindValue(':title',$_POST['UploadPhotoTitle']);
        $result->bindValue(':description',$_POST['UploadPhotoDescription']);
